...
export function BreedingPod({ onRequestGallery }: BreedingPodProps = {}) {
  ...
  return (
    <div className="flex flex-col items-center gap-3 p-4 relative" style={{ fontFamily: "'VT323', monospace" }}>
      {/* Mystic Altar Title - Moved further DOWN to sit on top of the altar */}
      <div className="h-16 flex flex-col items-center justify-center z-20 scale-125 min-w-[200px] translate-y-20">
         <RitualInscription sparkles={12} blueGlow={false} fontSize="12px">
           Mystic Altar
         </RitualInscription>
      </div>

      {state.mutationJuiceActive && (
        <div className="text-[9px] text-[#FF7EB6] bg-[#FF7EB6]/5 px-2 py-0.5 rounded animate-pulse mt-20 shadow-[0_0_10px_#40E0D0]/20 backdrop-blur-sm border border-[#FF7EB6]/20">
      ...
      <div className="relative w-[28rem] h-[14rem] flex items-center justify-center">
        <div className="relative w-full h-full z-10">
          ...
          <div
            className={`absolute left-[0%] top-[15%] w-32 h-32 flex items-center justify-center transition-all cursor-pointer ${breeding ? 'animate-spirit-pulse' : ''}`}
          ...
          <div
            className={`absolute right-[0%] top-[15%] w-32 h-32 flex items-center justify-center transition-all cursor-pointer ${breeding ? 'animate-spirit-pulse' : ''}`}
            onDragOver={handleDragOver}
            onDrop={handleDrop(2)}
          ...
          {/* CENTRAL GAMIFIED ACTIONS - Using a Floating Soul Gem */}
